...
{
    internal static class Task_BuyDronebitItems
    {
        ...
        private static bool? BuyItems()
        {
            if (GenericHelpers.TryGetAddonMaster<SelectYesno>("SelectYesno", out var yesno) && yesno.IsAddonReady)
            {
                if (EzThrottler.Throttle("OizysBag_ConfirmBuy", 500))
                {
                    ...
                    if (_pendingBuyAmount > 0 && C.Tmp_OizysDronebitBuyAmount > 0)
                    {
                        C.Tmp_OizysDronebitBuyAmount -= _pendingBuyAmount;
                        if (C.Tmp_OizysDronebitBuyAmount < 0)
                            C.Tmp_OizysDronebitBuyAmount = 0;
            ...
            if (currency < C.Tmp_OizysDronebitBuyAtAmount)
                return true;
            ...
            if (C.Tmp_OizysDronebitKeepBuying)
            {
                target = int.MaxValue;
            }
            else
            {
                if (C.Tmp_OizysDronebitBuyAmount > 0)
                    target = C.Tmp_OizysDronebitBuyAmount;

                else if (C.Tmp_OizysDronebitKeepAmount > currentCount)
                    target = C.Tmp_OizysDronebitKeepAmount - currentCount;
            }
        ...
        public static bool CanPurchaseAnyItem()
        {
            ...
            if (C.Tmp_OizysDronebitKeepAmount > currentCount)
                return true;
